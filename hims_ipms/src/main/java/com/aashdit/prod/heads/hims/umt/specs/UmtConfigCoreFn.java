...
import com.aashdit.prod.heads.hims.umt.dto.EntityHierarchy;
...
import com.aashdit.prod.heads.hims.umt.dto.RoleEntityDetails;
import com.aashdit.prod.heads.hims.umt.model.*;
import com.aashdit.prod.heads.hims.umt.repository.*;
...
public class UmtConfigCoreFn {
    ...
    private EntityHierarchy makingHierarchyEntity(NativeQueryResultsParent results, Long coId) {
        if(coId == null || coId < 1) {
            ...
            String type = split[1];
            // if in hierarchyEntityMapRepository there is already parentId and parentType then remove it from parentResult
...
//            }
        ...
        entityHierarchy.setLevelIdAndTypeAndName(results.getId() + "##" + results.getEntityCode() + "##" + results.getEntityName());
    ...
    private static EntityHierarchy makingChildEntities(HierarchyEntityMap hirEntMap1) {
        ...
        entityHierarchy1.setLevelIdAndTypeAndName(hirEntMap1.getObjectId() + "##" + hirEntMap1.getObjectType() + "##" + hirEntMap1.getObjectName());
    ...
    // get that field column name which is annotated with dynamic annotation passed as parameter
            ...
            for (EntityType<?> entity : entities) {
                ...
                if (entity.getJavaType().isAnnotationPresent(javax.persistence.Entity.class)) {
                    if (entity.getJavaType().isAnnotationPresent(javax.persistence.Table.class)) {
                        javax.persistence.Table table = entity.getJavaType().getAnnotation(javax.persistence.Table.class);
    ...
    public void addEntityIdAndLevelToModelWithData(Model model, Class<?> classObject, boolean isMandatory, String selectedEntityId, boolean isReadOnly, boolean isOnchangeFnNeeded) {
        try {
            ...
            if (model.containsAttribute(CommonUMTConstants.selectedEntityObjectIdAndType)) {
                ...
                if (model.getAttribute(CommonUMTConstants.selectedEntityObjectIdAndType) != null) {
                    selectedEntityId = Objects.requireNonNull(model.getAttribute(CommonUMTConstants.selectedEntityObjectIdAndType)).toString();
                }
            }
            ...
            roleService.getRoleRightLevelMasterByLevelKeyClass(classObject.getSimpleName() + ".class").ifPresent(roleRightLevelMaster -> {
    ...
    public String[] otherRolesToEntity(Class<?> entityClass, Long entityId) {
        Optional<RoleRightLevelMaster> lvlMst = roleService.getRoleRightLevelMasterByLevelKeyClass(entityClass.getSimpleName() + ".class");
        if (lvlMst.isPresent()) {
            List<RoleEntityMap> roleList = roleService.getEntitySpecificRoleEntityMapList(entityId, lvlMst.get().getLevelCode());
    ...
    public List<EntityHierarchy> getParentEntities(long entityId, String entityLevel, String entityClassStr, Long selectedObjectId) {
        List<EntityHierarchy> entityHierarchyList = new ArrayList<>();
        try {
            Class<?> entityClass = Class.forName(entityClassStr);
            Optional<RoleRightLevelMaster> lvlMst = roleService.getRoleRightLevelMasterByLevelKeyClass(entityClass.getSimpleName() + ".class");
            if (lvlMst.isPresent()) {
                List<OrganizationStructureHierarchy> hierarchyList = organizationStructureHierarchyRepository.findByLevelCodeAndOrgStrObjIdAndType(lvlMst.get().getLevelCode(), entityId, entityLevel);
                ...
                for (OrganizationStructureHierarchy hir : hierarchyList) {
                    if (hir != null) {
                        ...
                        List<HierarchyEntityMap> removeHierarchyEntityMap = hierarchyEntityMapRepository.findByOrganizationStructureHierarchyEqualsAndObjectTypeEqualsAndObjectIdNotIn(hir, lvlMst.get().getLevelCode(), objectIds);
                        removeHierarchyEntityMapAll.addAll(removeHierarchyEntityMap);
                    }
                }
                // remove all parent entities which are already added in hierarchyEntityMapRepository
                for (HierarchyEntityMap hirEntMap : removeHierarchyEntityMapAll) {
                    entityHierarchyList.removeIf(entityHierarchy -> entityHierarchy.getLevelIdAndType().equals(hirEntMap.getParentObjectIdAndType()));
                }
            }
        } catch (Exception e) {
    ...
    private void makingParentEntities(Long coId, OrganizationStructureHierarchy hir, List<NativeQueryResultsParent> resultsList, RoleRightLevelMaster lvlMst, List<EntityHierarchy> entityHierarchyList) {
        for (NativeQueryResultsParent results : resultsList) {
            ...
            String co = "";
            if (coId != null && coId > 0) {
                Optional<HierarchyEntityMap> hmp = hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityTypeAndPartIdAndType(hir, coId, lvlMst.getLevelCode(), results.getId(), results.getEntityCode());
                if (hmp.isPresent()) {
    ...
    public List<EntityHierarchy> getChildListEntityHierarchyList(long entityId, String entityLevel, Long selectedObjectId) {
        ...
        try {
            ...
            if (lvlMst.isPresent()) {
                ...
                Optional<HierarchyEntityMap> hirEntMap = hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityType(entityId, entityLevel);
                hirEntMap.ifPresent(allHierarchyEntityMap::add);
                // recursive call to get all child entities of child entities of child entities and so on
                ...
                for (HierarchyEntityMap hirEntMap1 : allHierarchyEntityMap) {
                    ...
                    if (selectedObjectId != null && selectedObjectId > 0) {
                        Optional<HierarchyEntityMap> hmp = hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityTypeAndPartIdAndType(hirEntMap1.getOrganizationStructureHierarchy(), selectedObjectId, lvlMst.get().getLevelCode(), hirEntMap1.getObjectId(), hirEntMap1.getObjectType());
                        if (hmp.isPresent()) {
                            co = hmp.get().getParentObjectId() + "##" + hmp.get().getParentObjectType();
                        }
                    }
            ...
            log.error("Error while fetching entity id and user level on self level UserServiceImpl::getEntityIdAndUserLevelOnSelfLevel() : ", e);
        }
        return entityHierarchy;
    }

    private void getChildListHierarchyEntityMapList(List<HierarchyEntityMap> allHierarchyEntityMap, String parentLevel, long parentId) {
        List<HierarchyEntityMap> childHierarchyEntityMapList = hierarchyEntityMapRepository.findByParentObjectIdAndParentObjectType(parentId, parentLevel);
        for (HierarchyEntityMap hirEntMap : childHierarchyEntityMapList) {
            Optional<RoleRightLevelMaster> lvlMst = roleService.getRoleRightLevelMasterByCode(hirEntMap.getObjectType());
            if (lvlMst.isPresent()) {
                if (lvlMst.get().getMappingType().equalsIgnoreCase("ONE-TO-ONE")) {
                    ...
                    boolean isAlreadyAdded = allHierarchyEntityMap.stream().anyMatch(hirEntMap1 -> hirEntMap1.getObjectId().equals(hirEntMap.getObjectId()) && hirEntMap1.getObjectType().equals(hirEntMap.getObjectType()));
                    if (!isAlreadyAdded) {
                        allHierarchyEntityMap.add(hirEntMap);
                        getChildListHierarchyEntityMapList(allHierarchyEntityMap, hirEntMap.getObjectType(), hirEntMap.getObjectId());
                    }
                }
            }
        }
        ...
        try {
            ...
            if (hirEntMap.isPresent()) {
                ...
                if (isTopToBottom) {
                    ...
                    Optional<OrganizationStructureHierarchy> parentHierarchy = organizationStructureHierarchyRepository.findById(hierarchy.getParentHierarchyId());
                ...
                for (OrganizationStructureHierarchy orgHir : hierarchyList) {
                    List<HierarchyEntityMap> entityMapList = hierarchyEntityMapRepository.findByOrganizationStructureHierarchy(orgHir);
                    for (HierarchyEntityMap entityMap : entityMapList) {
                        ...
                        entityHierarchy.setLevelName(entityMap.getObjectName() + " (" + entityMap.getObjectType() + ")");
    ...
    public List<RoleEntityCommonDto> getRoleListByEntityId(long entityIdLong, String entityLevel) {
        ...
        /*List<Role> publicList = roleRepository.findByRoleAccessType(CommonUMTConstants.ROLE_ACCESS_TYPE_PUBLIC);
        for (Role role : publicList) {
            RoleEntityCommonDto dto = new RoleEntityCommonDto();
            dto.setRoleId(role.getRoleId());
            dto.setRoleCode(role.getRoleCode());
            dto.setEntityLevel(entityLevel);
            String combineValue = role.getDisplayName() + " || " + role.getRoleCode() + " || " + entityLevel;
            dto.setCombineValue(combineValue);
            dto.setDisplayName(role.getDisplayName());
            result.add(dto);
        }*/
        Set<RoleEntityCommonDto> set = result.stream().collect(Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(RoleEntityCommonDto::getRoleId))));
