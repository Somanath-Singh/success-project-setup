...
public class RoleServiceImpl implements RoleService, MessageSourceAware {
    ...
    public ServiceOutcome<Role> addNupdateRole(Role role) {
        ...
        try {
            ...
            if (role.getRoleId() != null) {
                ...
                outcome.setMessage(messageSource.getMessage("role.msg.update.success", null, "Role updated successfully", LocaleContextHolder.getLocale()));
            } else {
                // role=new Role();
                ...
                //Make role code from display name by adding trim then ROLE_ prefix and converting to uppercase and replacing spaces with underscor
                ...
                if(byRoleCode != null) {
                    ...
                    outcome.setMessage(messageSource.getMessage("role.msg.duplicate.error", null, "Role already exists", LocaleContextHolder.getLocale()));
                    return outcome;
                }
                ...
                outcome.setMessage(messageSource.getMessage("role.msg.save.success", null, "Role saved successfully", LocaleContextHolder.getLocale()));
            }
        } catch (Exception e) {
            log.error(e);
            outcome.setData(null);
    ...
    public Boolean roleEntityMap(String[] roleCodes, Class<?> entityClassName, Long id) {
        for (String roleCode : roleCodes) {
            Role role = roleRepository.findByRoleCode(roleCode);
            if(role != null) {
                Optional<RoleRightLevelMaster> lvlMst = roleRightLevelMasterRepository.findByLevelKeyClass(entityClassName.getSimpleName() + ".class");
                if (lvlMst.isPresent()) {
    ...
    private String[] roleEntityAddAndUpdate(Role role) {
        ...
        Optional<RoleEntityMap> level = roleEntityMapRepository.findByEntityIdAndLevelRoleCode(Long.parseLong(split[0]), split[1], role.getRoleCode());
        if(level.isPresent()) {
    ...
    public List<Role> getEntitySpecificSelectedRoleList(Long selectedEntityId, String selectedEntityLevel) {
        List<Role> roles = new ArrayList<>();
        List<RoleEntityMap> entityMapList = roleEntityMapRepository.findByEntityIdAndLevel(selectedEntityId, selectedEntityLevel);
        if (!entityMapList.isEmpty()) {
            for (RoleEntityMap roleEntityMap : entityMapList) {
                Role role = roleRepository.findByRoleCode(roleEntityMap.getRoleCode());
                if (role == null) {
	...
	public List<Role> getAllRollsByUser() {
		List<Role> roles = roleRepository.findAllPublicRoles();
        List<RoleEntityMap> entityMapList = roleEntityMapRepository.findByEntityIdAndLevel(SecurityHelper.getCurrentUserEntityId(), SecurityHelper.getCurrentUserEntityLevel());
        if (!entityMapList.isEmpty()) {
            for (RoleEntityMap roleEntityMap : entityMapList) {
                Role role = roleRepository.findByRoleCode(roleEntityMap.getRoleCode());
                if (role == null) {
    ...
    public List<String> getAllRoleCodesActiveForMobile() {
        ...
        try {
            roleCodes = roleRepository.findByIsActiveTrueAndIsRoleApplForMobileTrue()
                    .stream()
