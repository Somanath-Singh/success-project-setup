...
import com.aashdit.prod.heads.hims.umt.repository.HierarchyEntityMapMultipleRepository;
...
import com.aashdit.prod.heads.hims.umt.repository.PriviledgeRepository;
import com.aashdit.prod.heads.hims.umt.repository.RoleMenuMapRepository;
import com.aashdit.prod.heads.hims.umt.repository.RoleMenuPrivilegeMapRepository;
...
import com.aashdit.prod.heads.hims.umt.specs.ObjectEntityId;
import com.aashdit.prod.heads.hims.umt.specs.ObjectEntityLevel;
...
public class PriviledgeServiceImpl implements PriviledgeService, MessageSourceAware {
    ...
    private static NativeQueryResultsParent makingParentNativeObject(HierarchyEntityMap entityMap, String mappingType) {
        ...
        nativeQueryResult.setParentEntityIdAndCode(nativeQueryResult.getParentEntityId() + "##" + nativeQueryResult.getParentEntityCode());
    ...
    public boolean saveOrganizationStructure(String organizationStructureArr) {
        try {
            ...
            Optional<OrganizationStructure> orgStrExt = organizationStructureRepository.findByObjectIdAndObjectType(entityId, level);
    ...
    private void saveOrgStrHierarchy(JsonObject jsonObject, Long id, Long parentId) {
        ...
        if(parentId == null){
            ...
            Optional<HierarchyEntityMap> hierarchy = hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityType(orgStrHierarchy, orgNameId, orgStrHierarchy.getLevelCode());
            ...
            /*try {
                RoleRightLevelMaster byLevelCode = roleRightLevelMasterRepository.findByLevelCode(munLevel);
                String tableName = byLevelCode.getLevelEntityName();
                // className is like MstMunicipality.class convert it to MstMunicipality class
                Class<?> clazz = umtConfigCoreFn.getClassFromTableName(tableName);
                String parentEntityTypeStr = umtConfigCoreFn.getParentObjectField(clazz, ParentObjectLevel.class);
                String parentEntityIdStr = umtConfigCoreFn.getParentObjectField(clazz, ParentObjectId.class);

                if (parentEntityType != null && parentEntityId != null) {
                    parentEntityId = Long.parseLong(parentEntityIdStr);
                    parentEntityType = parentEntityTypeStr;
                }
            }catch (Exception e){
                log.error("Error in saveOrgStrHierarchy", e);
            }*/
    ...
    public List<OrganizationStructureParentChild> viewOrganizationStructure(String filterData) throws Exception {
        ...
        try {
            ...
            String type = split[1];
            Optional<OrganizationStructure> orgStr = organizationStructureRepository.findByObjectIdAndObjectType(id, type);
            if (!orgStr.isPresent()) {
    ...
    @Override
    public List<NativeQueryResults> getEntitySpecificEntity(String entityIdAndTypeEncode, String levelCode, String orgIdEncode, String levelHeight) {
        ...
        try {
            ...
            Optional<OrganizationStructureHierarchy> orgStrHierarchy = organizationStructureHierarchyRepository.findById(orgId);
            ...
            Optional<UmtNativeQuery> nativeQuery = umtNativeQueryRepository.findByQueryCode(levelCode + CommonUMTConstants.OBJECT_ID_AND_TYPE);
            ...
            for (Object[] objects : resultList) {
                ...
                Optional<HierarchyEntityMap> entityMap = hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityType(entityId, entityLevel);
    ...
    public List<NativeQueryResultsParent> getParentResultSet(OrganizationStructureHierarchy orgStrHierarchy) {
        ...
        if (lvlMst != null) {
            ...
            OrganizationStructureHierarchy parentHierarchy = orgStrHierarchy.getParentHierarchyId() == null ? orgStrHierarchy : organizationStructureHierarchyRepository.getOne(orgStrHierarchy.getParentHierarchyId());
            List<HierarchyEntityMap> entityMaps = hierarchyEntityMapRepository.findByOrganizationStructureHierarchy(parentHierarchy);
    ...
    public List<NativeQueryResultsParent> getChildResultSet(OrganizationStructureHierarchy orgStrHierarchy) {
        ...
        if (lvlMst != null) {
            String mappingType = lvlMst.getMappingType();
          //  OrganizationStructureHierarchy childHierarchy = orgStrHierarchy.getChildHierarchyId() == null ? orgStrHierarchy : organizationStructureHierarchyRepository.getOne(orgStrHierarchy.getChildHierarchyId());
        }
        return parentResult;
    }
    ...
    public String getParentObjectIdAndType(long entityId, String entityLevel) {
        ...
        try {
            Optional<HierarchyEntityMap> entityMap = hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityType(entityId, entityLevel);
            if (entityMap.isPresent()) {
                parentObjectIdAndType = entityMap.get().getParentObjectId() + "##" + entityMap.get().getParentObjectType();
            }
        }catch (Exception e){
        ...
        try {
            ...
            if (lvlMst != null) {
                ...
                if (parentEntityLevelStr != null && parentEntityIdStr != null) {
                    String sql = "SELECT " + parentEntityIdStr + ", " + parentEntityLevelStr + " FROM " + tableName + " WHERE " + id + " = " + entityId;
                    Query query = entityManager.createNativeQuery(sql);
                    List<Object[]> resultList = query.getResultList();
                    for (Object[] objects : resultList) {
    ...
    private JsonObject getChildHierarchyView(Long parentId, String parentType) {
        JsonObject jsonObject = new JsonObject();
        try {
            List<HierarchyEntityMap> child = hierarchyEntityMapRepository.findByParentObjectIdAndParentObjectType(parentId, parentType);
            JsonArray jsonArray = new JsonArray();
            for (HierarchyEntityMap entityMap : child) {
                JsonObject childHierarchyView = getChildHierarchyView(entityMap.getObjectId(), entityMap.getObjectType());
    ...
    private JsonObject getParentHierarchyView(long entityId, String entityType, boolean isParticularEntity) {
        JsonObject jsonObject = new JsonObject();
        try {
            Optional<HierarchyEntityMap> hierarchyEntityMap = hierarchyEntityMapRepository.findByEntityIdAndEntityType(entityId, entityType);
            if (hierarchyEntityMap.isPresent()) {
                ...
                // if objectId, objectType and parentObjectId, parentObjectType are same then return
    ...
    private NativeQueryResultsParent makingNativeQueryResultParent(Object[] objects) {
        NativeQueryResultsParent nativeQueryResult = new NativeQueryResultsParent();
        nativeQueryResult.setId(objects.length > 0 && objects[0] != null ? ((BigInteger) objects[0]).longValue() : null);
        ...
        nativeQueryResult.setParentEntityId(objects.length > 3 && objects[3] != null ? ((BigInteger) objects[3]).longValue() : null);
    ...
    private String getEntityIdsForHierarchy(OrganizationStructureHierarchy parentHierarchy) {
        List<HierarchyEntityMap> entityMaps = hierarchyEntityMapRepository.findByOrganizationStructureHierarchy(parentHierarchy);
        return entityMaps.stream().map(HierarchyEntityMap::getObjectId).map(String::valueOf).map(s -> "'" + s + "'").collect(Collectors.joining(","));
    }


    private NativeQueryResults makingNativeQueryResult(Object[] objects, List<NativeQueryResultsParent> parentResult, OrganizationStructureHierarchy hirChy) {
        ...
        hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityType(hirChy, nativeQueryResult.getId(), hirChy.getLevelCode()).ifPresent(entityMap -> {
            nativeQueryResult.setParentEntityId(entityMap.getParentObjectId());
            nativeQueryResult.setParentEntityCode(entityMap.getParentObjectType());
            nativeQueryResult.setParentEntityIdAndCode(nativeQueryResult.getParentEntityId() + "##" + nativeQueryResult.getParentEntityCode());
    ...
    @Transactional
    @Override
    public Boolean saveEnt(String[] upperEntityIdAndType, String entityClass, String objectTypeAndId, Long entityId, String entityName){
        ...
        if (lblMst.isPresent()) {
            ...
            if(lvl.getMappingType().equalsIgnoreCase("ONE-TO-ONE")){
                if (upperEntityIdAndType != null && upperEntityIdAndType.length > 0) {
                    ...
                    JsonObject jsonObject = makingJsonForHmp(objectTypeAndId, entityId, entityName, upperEntityIdAndTypeStr, lvlCode, lvl);
    ...
    private JsonObject makingJsonForHmp(String objectTypeAndId, Long entityId, String entityName, String upperEntityIdAndTypeStr, String lvlCode, RoleRightLevelMaster lvl) {
        JsonObject jsonObject = new JsonObject();
        if(upperEntityIdAndTypeStr != null && !upperEntityIdAndTypeStr.isEmpty()) {
            ...
            List<OrganizationStructureHierarchy> allParents = organizationStructureHierarchyRepository.findByLevelCodeAndOrgStrObjIdAndType(upperEntityType, objectId, objectType);
            ...
            for (OrganizationStructureHierarchy parent : allParents) {
                Optional<OrganizationStructureHierarchy> currentClassEnt1 = organizationStructureHierarchyRepository.findByLevelCodeAndParentId(lvlCode, parent.getId());
        ...
        try {
            ...
            for (JsonElement jsonElement : selectedEntityIds) {
                ...
                if (roleRightLevelMaster == null) {
                    throw new Exception("RoleRightLevelMaster not found for PriviledgeServiceImpl.saveEntitySpecificEntity");
                }
                Optional<OrganizationStructureHierarchy> orgStrHierarchy = organizationStructureHierarchyRepository.findById(orgHigId);
                if (!orgStrHierarchy.isPresent()) {
                    throw new Exception("OrganizationStructureHierarchy not found for PriviledgeServiceImpl.saveEntitySpecificEntity");
                }
                ...
                if (mappingType.equalsIgnoreCase("ONE-TO-MANY")) {
                    for (JsonElement parentEntityIdAndType : parentEntityIdAndTypeArr) {
                        saveEntityHirMap(parentEntityIdAndType.getAsString(), parentEntityNameArr, clazz, tableName, entityId, jsonObject, orgStrHierarchy, levelCode, entityName, false);
                    }
                }
                else if (mappingType.equalsIgnoreCase("ONE-TO-ONE")) {
                    String parentEntityIdAndType = parentEntityIdAndTypeArr.get(0).getAsString();
                    saveEntityHirMap(parentEntityIdAndType, parentEntityNameArr, clazz, tableName, entityId, jsonObject, orgStrHierarchy, levelCode, entityName, true);
                }
            }
    ...
    @Transactional
    private void saveEntityHirMap(String parentEntityIdAndType, JsonArray parentEntityNameArr, Class<?> clazz, String tableName, long entityId, JsonObject jsonObject, Optional<OrganizationStructureHierarchy> orgStrHierarchy, String levelCode, String entityName, boolean runForParentUpdate) {
        ...
        HierarchyEntityMap hierarchyEntityMap = new HierarchyEntityMap();
        Optional<HierarchyEntityMap> hierarchy = hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityTypeAndPartIdAndType(orgStrHierarchy.get(), entityId, levelCode, parentEntityId, parentEntityType);
        if (hierarchy.isPresent()) {
            hierarchyEntityMap = hierarchy.get();
   ...
   /* @Transactional // Ensures that the transaction is active and commits automatically
    public void updateEntitySpecificEntity(Class<?> clazz, String tableName, String parentEntityType, Long parentEntityId, Long entityId, JsonObject jsonObject) {
        log.info("Transaction active: " + TransactionSynchronizationManager.isActualTransactionActive());
        try {
            String tableIdName = umtConfigCoreFn.getTableIdName(clazz);
            String updateQuery = "UPDATE " + tableName + " SET parent_object_type = ?, parent_object_id = ? WHERE " + tableIdName + " = ?";
            log.info("updateQuery: " + updateQuery);
            int update = jdbcTemplate.update(updateQuery, parentEntityType, parentEntityId, entityId);
            if (update == 0) {
                throw new Exception("Error in updateEntitySpecificEntity");
            }
    ...
    // recursive function to get the data from db and create the json object like above
    private JsonArray viewOrgStrHierarchy(Long id, Long parentId) {
        JsonArray jsonArray = new JsonArray();
        List<OrganizationStructureHierarchy> orgStrHierarchyList;
        if (parentId == null) {
            orgStrHierarchyList = organizationStructureHierarchyRepository.findByOrganizationStructureIdAndParentNull(id);
        } else {
    ...
    // recursive function to return get the data from db and create the Map object only value - String and children - Map
    private List<OrganizationStructureParentChild> viewOrgStrHierarchy(Long id, Long parentId, List<OrganizationStructureParentChild> orgStrHierarchyList) {
        List<OrganizationStructureHierarchy> orgStrHierarchyList1;
        if (parentId == null) {
            orgStrHierarchyList1 = organizationStructureHierarchyRepository.findByOrganizationStructureIdAndParentNull(id);
        } else {
