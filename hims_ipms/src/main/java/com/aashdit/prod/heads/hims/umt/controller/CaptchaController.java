...
import com.aashdit.prod.heads.framework.core.captcha.CaptchaMaker;
import com.aashdit.prod.heads.framework.core.util.CaptchaString;
...
import com.aashdit.prod.heads.hims.umt.utils.PasswordValidator;
...
public class CaptchaController implements MessageSourceAware {
	...
	/*
	 * i - ignore | v - verify s - smallcase | u - uppercase | m - mixed
	 */
	...
	public void generateCaptcha(HttpServletRequest request, HttpServletResponse response, @PathVariable Integer size)
			throws IOException {
	...
	public String doLogin(HttpServletRequest request, @ModelAttribute AuthRequest authRequest,
			...
			if (svcOutcome.getOutcome() != false) {
				...
				if (user != null) {
					if (user.getIsEnabled()) {
						...
						if (user.getIsLocked()) {
							model.addFlashAttribute("error_msg", messageSource.getMessage("login.account.locked", null,
						...
						if (user.getIsLoggedIn() && user.getAllowMultipleSession() == false) {
							model.addFlashAttribute("error_msg", messageSource.getMessage("login.account.loggedin",
						...
						if (isOK) {
							...
							if (verifiyCaptcha) {
								...
								if (!svrCaptcha.equals(authRequest.getCaptcha())) {
									model.addFlashAttribute("error_msg", messageSource.getMessage(
											"login.account.badcaptcha", null, LocaleContextHolder.getLocale()));
						...
						if (isOK) {
							...
							if (!isOK) {
								model.addFlashAttribute("error_msg", messageSource.getMessage(
										"login.account.badcredentials", null, LocaleContextHolder.getLocale()));
						...
						if (isOK) {
							...
							user.setWrongLoginCount(
									(user.getWrongLoginCount() == null ? 0 : user.getWrongLoginCount()) + 1);
						...
						model.addFlashAttribute("error_msg", messageSource.getMessage("login.account.disabled", null,
								LocaleContextHolder.getLocale()));
						logger.debug("Account Disabled");
						isOK = false;
					}
				} else {
					model.addFlashAttribute("error_msg", messageSource.getMessage("login.account.badcredentials", null,
							LocaleContextHolder.getLocale()));
	...
	private void authorizeUser(HttpServletRequest request, User user, String appCode) {
		...
		// load active user role level
		// customLoginController.loadActiveUserRoleLevel(request, userDetails);

		// loadMenus(request, userDetails, appCode);
		// customLoginController.loadModulesBasedOnPrimaryRole(request, userDetails);
	...
	public String doLogout(HttpServletRequest request, HttpServletResponse response) throws MalformedURLException {
		...
		// return "redirect:" + request.getProtocol().split("/")[0].toLowerCase() +
	...
	public String switchRole(RedirectAttributes model, @RequestParam Long roleId,
			@RequestParam(required = false) Long entityId, @RequestParam(required = false) String appCode,
		...
		try {
			...
			if (svcUser.getOutcome()) {
				if (entityId != null) {
					// customLoginController.updateUserRoleLevelAccess(entityId);
				}
			...
			model.addFlashAttribute("err_msg",
					messageSource.getMessage("msg.error", null, LocaleContextHolder.getLocale()));
			logger.error(ex);
		}
	...
	private void loadMenus(HttpServletRequest request, LoggedInUser liu, String appCode) {
		...
		if (appCode != null) {
			m_menus = m_menus.stream().filter(e -> e.getAppCode() == null || e.getAppCode().equals(appCode))
	...
	private List<Menu> getMenuChildren(Menu parent) {
		List<Menu> children = m_menus.stream()
				...
				.filter(m -> m.getIsDisplay() == true).collect(Collectors.toList());

		try {
			if (children != null) {
				children = children.stream().sorted((a, b) -> (a.getDisplayOrder() - b.getDisplayOrder()))
						.collect(Collectors.toList());
	...
	@PostMapping(path = "/umt/user/captcha/change/password", name = "Change Password")
	public String changePassword(RedirectAttributes attr, String txtPass, String txtRePass, HttpServletRequest request,
			...
			if (txtRePass != null && !txtRePass.trim().isEmpty() && txtPass != null && !txtPass.trim().isEmpty()) {
				...
				if (chk) {
					if (txtPass.equals(txtRePass)) {
						...
						if (isSuccess) {
							return this.doLogout(request, response);
						} else {
							return "redirect:" + POST_LOGIN_URL;
						}
					}
				} else
					throw new Exception("Password does not meet the policy");
			} else
				throw new Exception("Password not changed");
		} catch (Exception e) {
			logger.error(e.getMessage());
			attr.addFlashAttribute("error_msg", e.getMessage());
			return "redirect:" + POST_LOGIN_URL;
		}
		return "redirect:" + POST_LOGIN_URL;
	}

	@PostMapping(value = "/umt/reset/forgot-password-send-otp", name = "Reset Password")
	public String forgotPassword(RedirectAttributes attribute, String username) {
		try {
			ServiceOutcome<Boolean> result = userService.forgotPasswordSendOtp(username);
			attribute.addFlashAttribute(result.getData() ? "success_msg" : "err_msg", result.getMessage());
		} catch (Exception e) {
			logger.error("Exception occurred in forgotPassword in CaptchaController-->" + e);
		}
		return "redirect:/login";
	}
