package com.aashdit.prod.heads.framework.core.captcha;
...
public class MyPainter {
	...
	public static enum Quality {
		...
		MIN,
		/**
		 * Rendering hints should be not set so they use the default. quality
		 */
		DEFAULT,
		/**
	 ...
	 * Constructor.
	 * 
	 * @param rnd random generator to be used, can be null
	 */
	public MyPainter(Random rnd) {
	 ...
	 * Constructor.
	 * 
	 * @param width        captcha image width, default {@link #DEFAULT_WIDTH}
	 * @param height       captcha image height, default {@link #DEFAULT_HEIGHT}
	 * @param bGround      background color of captcha image, default white, can be
	 *                     null
	 * @param quality      captcha image quality, default {@link Quality#MAX},
	 *                     should use max it does not have measurable speed penalty
	 *                     on modern jvm-s (1.6u23), can be null
	 * @param effectConfig used to define what effects should be used, can be null
	 * @param rnd          random generator to be used, can be null
	 */
	public MyPainter(int width, int height, Color bGround, Quality quality, EffectConfig effectConfig, Random rnd) {
		super();
		this.width = width;
		this.height = height;
	 ...
	 * 
	 * @param font    will be used for text, not null
	 * @param fGround will be used for text, not null
	 * @param text    this will be rendered on the image, not null, not 0 length
	 ...
	 * 
	 * @param g       to be configured must be an instance of {@link Graphics2D},
	 *                not null
	 * @param font    to be used for the text, not null
	 * @param fGround to be used for the text, not null
	 * 
	 ...
	 * 
	 * @param g2 to be configured, not null
	 */
	 ...
	 * {@link #transform(Graphics2D, String, GlyphVector)}), scales, transforms and
	 * draws the result (also the outline if needed).
	 * 
	 * @param g    to be drawn upon
	...
	protected void draw(Graphics2D g, String text) {
		...
		final float wr = width / bw * (rnd.nextFloat() / 20 + (outlineEnabled ? 0.89f : 0.92f))
				* effectConfig.getScaleConfig().getX();
		final float hr = height / bh * (rnd.nextFloat() / 20 + (outlineEnabled ? 0.68f : 0.75f))
		...
		if (outlineEnabled)
			g.draw(vector.getOutline(Math.signum(rnd.nextFloat() - 0.5f) * 1 * width / 200 - bx,
					Math.signum(rnd.nextFloat() - 0.5f) * 1 * height / 70 + height - by));
	 ...
	 * @param g    to be drawn upon
	 * @param text to be drawn
	 * @param v    graphical representation of text, to be transformed
	 */
	protected void transform(Graphics2D g, String text, GlyphVector v) {
		...
		for (int fi = 0; fi < glyphNum; fi++) {
			...
			if (prePos == null)
				...
				newPos = new Point2D.Double(preBounds.getMaxX() + pos.getX() - bounds.getX()
						- Math.min(preBounds.getWidth(), bounds.getWidth())
								* (rnd.nextDouble() / 20 + (rotateEnabled ? 0.27 : 0.1)),
						pos.getY());
	 ...
	 * (waving) and blurring.
	 * 
	 * @param img to be post prosessed.
	 ...
	 * 
	 * @param array filled with random values. The values in array sum up to 1.
	 */
