...
import com.aashdit.prod.heads.framework.core.util.PasswordValidator;
import com.aashdit.prod.heads.framework.core.util.ViewDocuments;
...
public class UserController {
	...
	public String userRegistration(RedirectAttributes attr, User user) {
		...
		ServiceOutcome<User> userServiceOutcome = userService.userRegistration(user, roleCode);
	...
	public String userList(Model model, @RequestParam(required = false) String searchTerm,
	...
	public String postUserList(Model model, @RequestParam(required = false) String searchTerm,
			@RequestParam(required = false) Integer page, @RequestParam(required = false) Integer size) {
	...
	public String userUpdate(RedirectAttributes attr, Long userid, String username, String firstname, String lastname,
			...
			@RequestParam("isPrimary[]") Long isPrimary[],
			@RequestParam(value = "status[]", required = false) String status[],
			@RequestParam("userRoleHcMapId[]") Long userRoleHcMapId[], @RequestParam String designation) {

		ServiceOutcome<User> serviceOutcome = userService.updateUser(userid, username, firstname, lastname, dateOfbirth,
				userMobile, userEmail, userRoleHcMapId, roleName, isPrimary, status, designation);
		if (serviceOutcome.getOutcome()) {
			attr.addFlashAttribute("success_msg", serviceOutcome.getMessage());
	...
	public String addUser(RedirectAttributes attr, String username, String firstname, String lastname, Date dateOfbirth,
			String userMobile, String userEmail, @RequestParam("roleName[]") Long roleName[],
			@RequestParam("isPrimary[]") Long isPrimary[], @RequestParam String designation) {

		ServiceOutcome<User> serviceOutcome = userService.addUser(username, firstname, lastname, dateOfbirth,
				userMobile, userEmail, roleName, isPrimary, designation, "", designation, 0L, new Long[] { 0L }, null);
