		...
		<div class="col-md-6">
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active" aria-current="page">Add And
						Update Section</li>
				</ol>
			</nav>
		</div>
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				...
				<div class="card-body">
					<form action="${contextPath}/section/save" method="post"
						id="saveFormId" class="row">
						<input type="hidden" name="${_csrf.parameterName}"
							value="${_csrf.token}" /> <input type="hidden" name="sectionId"
							value="${section.sectionId}" id="sectionId" />

						<c:set var="functionList" value="getRoleList(this)" />
						<%@ include
							file="/WEB-INF/views/masters/entityAndUser/entityIdAndUserLevelListForMst.jsp"%>
						<div class="form-group col-md-3">
							<label class="required">Section Name :</label> <input type="text"
								class="form-control form-control-sm require" id="sectionName"
								name="sectionName" maxlength="20" value="${section.sectionName}">
						</div>

						<div class="form-group col-md-3">
							<label class="required">Section Code :</label> <input type="text"
								class="form-control form-control-sm capitalizeEachLetter require"
								id="sectionCode" name="sectionCode" maxLength="20"
								value="${section.sectionCode}"
								<c:if test="${!empty section.sectionCode}">readonly</c:if> />
						</div>

						<div class="form-group col-md-3">
							<label class="required" for="departmentId">Department
								Description :</label>
							<textarea class="form-control form-control-sm require"
								id="sectionDescription" name="sectionDescription"
								maxLength="100">${section.sectionDescription}</textarea>
						</div>
						...
						<div class="form-group col-md-12 text-center"
							...
							<button type="button" class="btn btn-submit"
								onclick="submitForm()">${empty section.sectionId ? 'Submit' : 'Update'}
							</button>
		...
		<div class="row mt-3">
			<div class="col-md-12">
				<div class="card">
					...
					<div class="card-body">
						<%@ include
							file="/WEB-INF/views/masters/entityAndUser/entityIdAndUserLevelListDrpForMst.jsp"%>
						<table
							class="datatable table table-striped table-bordered mt-5 exportbtn">
							<thead>
								<tr>
							...
							<tbody>
								<c:forEach items="${sectionMasterList}" var="sect"
									varStatus="count">
									<tr>
										...
										<td><a href="#"
											onclick="editSection('${sect.sectionId}')"
											class="btn btn-primary btn-xs" title="Edit"> <i
												class="fa fa-edit"></i>
										</a>
											<button type="button"
												class="btn ${!sect.isActive ? 'btn-danger' : 'btn-success'} btn-xs"
												data-toggle="tooltip"
												onclick="updateStatus(${sect.sectionId},${sect.isActive ?false:true})"
												title="${sect.isActive ? 'Activate' : 'Deactivate'}">
...
<form action="${contextPath}/section/update-status" method="post"
	id="updateStatusForm">
	<input type="hidden" name="${_csrf.parameterName}"
		value="${_csrf.token}" /> <input type="hidden" id="sectionIdd"
		name="sectionIdd" /> <input type="hidden" name="status"
		id="isActiveStatus" />
</form>
