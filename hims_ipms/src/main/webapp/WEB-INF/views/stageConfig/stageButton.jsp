...
<c:choose>
	<c:when test="${not empty buttonLabel}">
		<div class="col-lg-12 text-end btnholder no-print" style="padding-bottom:10px;">
			<input type="hidden" name="specificUserParamRemarks" id="specificUserIds" value="">
			<c:forEach items="${buttonLabel}" var="button">
				<button type="button" style="" onclick="buttonAction(this, ${button.stageId}, ${button.isUserSpecificRule}, ${button.isRemarksRequired}, '${button.buttonCode}')" class="${button.buttonColor} no-print unfreezeField">${button.buttonName}</button>
			</c:forEach>
			<!-- Back Button -->
			<button type="button" class="btn btn-warning no-print unfreezeField back" onclick="history.back()" ><i class="fa-solid fa-rotate-left"></i> Back</button>
		</div>
	</c:when>
	<c:otherwise>
		<div class="col-lg-12 text-end btnholder">
			<!-- Back Button -->
			<button type="button" class="btn btn-warning no-print unfreezeField back" onclick="history.back()"><i class="fa-solid fa-rotate-left"></i> Back</button>
		</div>
	</c:otherwise>
...
<div class="modal fade" id="userSpecificRuleModal" tabindex="-1" role="dialog" aria-labelledby="userSpecificRuleModalLabel" aria-hidden="true">
	...
	<input type="hidden" name="hdnButtonCode" id="hdnButtonCode" value="">
</div>
...
var wonFunction = "${wonFunction}";
	...
	function validationForAtleastOneGreaterThanZero(className) {
		...
		if(!isAtleastOneGreaterThanZero){
			...
			altleastOneGreaterThanZero.each(function(){
				this.classList.add("is-invalid");
				displayErrorMsgByFieldOnTitel(this, "At least one field must be greater than zero");
			});
		}else{
			altleastOneGreaterThanZero.each(function(){
				this.classList.remove("is-invalid");
...
function buttonAction(that, stageId, isUserSpecificRule, isRemarksRequired, buttonCode){
	...
	if(wonFunction != null && wonFunction != undefined && wonFunction != "") {

		let currentStageIdV2Html = addHiddenField("currentWorkflowStageParamName", stageId);
		dynamicFromOfWF.append(currentStageIdV2Html);
		...
		if(isRemarksRequired){
			bootbox.prompt({
				...
				callback: function (result) {
					if(result != null){
						let currentStageRemarkV2Html = addHiddenField("currentWorkflowStageParamRemarks", result);
						dynamicFromOfWF.append(currentStageRemarkV2Html);
						eval(wonFunction);
					}
				}
			});
		}else{
			// trigger the wonFunction
			eval(wonFunction);
		}
	}else{
		...
		var thisrequreFieldValidation = validateRequiredFields("thisRequiredField");
		if(!thisrequreFieldValidation){
			return;
		}
