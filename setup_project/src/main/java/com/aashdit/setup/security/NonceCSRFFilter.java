	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (!request.getMethod().equals("OPTIONS")) {
			...
			if (!this.requireCsrfProtectionMatcher.matches(request)) {
				...
				if (!request.getRequestURI().contains("/transaction/apply/")
						&& !request.getRequestURI().contains("/api/login")
						&& !request.getRequestURI().contains("/api/v1/save-fund-disbursement")
						&& !request.getRequestURI().contains("/api/expenditure/actionPlanFilter")
