...
public class SanitizationInterceptor implements HandlerInterceptor {
	...
	private static final Pattern FORMAT_SPECIFIER_PATTERN = Pattern.compile("%[a-zA-Z]", Pattern.CASE_INSENSITIVE); // Matches
																													// format
																													// specifiers
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		boolean hasInvalidInput = hasInvalidInput(request.getParameterMap())
				|| hasInvalidInput(request.getHeaderNames(), request) || hasInvalidInput(request.getCookies())
				|| hasInvalidInput(request.getRequestURI());

		if (hasInvalidInput) {
			String requestDetails = String.format("Request URI: %s, Parameters: %s, Headers: %s, Cookies: %s",
					request.getRequestURI(), request.getParameterMap(), Collections.list(request.getHeaderNames()),
	...
	private boolean hasInvalidInput(Map<String, String[]> parameterMap) {
		return parameterMap.values().stream().flatMap(Arrays::stream).anyMatch(this::containsInvalidContent);
	}
