...
public class UmtApplicationDateUtils {
	 ...
	 * @author Akash Behera
	 * @since 02-03-2024
	 * @return current time as hh:mi:ss (24Hr Format)
	 */
	public static String getStringNowAsHrMiSec24HrFormat() {
		Calendar ct = new GregorianCalendar();
		Date dt = ct.getTime();
	 ...
	 * @version 1.0
	 * @since 02-05-2017
	 * @return the financial year from given 2023-2024
	 */
	public static String convertToFinancialYear(String input) {
		String[] years = input.split("-");
		int startYear = Integer.parseInt(years[0]);
		int endYear = Integer.parseInt(years[1]);

		return String.format("%d/04/01 - %d/03/31", startYear, endYear);
	}
	...
	public static boolean isLastDayOfQuarter() {
		...
		boolean isQuarterMonth = currentDate.getMonthValue() == 1 || currentDate.getMonthValue() == 4
				|| currentDate.getMonthValue() == 7 || currentDate.getMonthValue() == 10;
	 ...
	 * @throws ParseException
	 * @description Returns the Retirement Date based on DOB (If DOB is on 1st, then
	 *              'x' years-1day, else last date of month falling after adding 'x'
	 *              years
	 */
	public static String getRetirementDateBasedOnDOB(String strDateOfBirth, int intRetirementAge)
			throws ParseException {
		...
		/**
		 * (3) ------------------ GET LAST DATE OF MONTH FALLING AFTER ADDING NO OF
		 * YEARS TO AN INPUT DATE -------------------
		 */
	 ...
	 * @descr For 01/01/2023, EndDate will be 31/01/2023, For 15/01/2023, it will be
	 *        14/02/2023 and so on ...
	 */
	 ...
	 * @throws ParseException
	 */
	public static Map<Integer, Map<Integer, Integer>> getMonthYearListForArrearSalary(String fromDate, String tillDate)
			throws ParseException {
		Date dtFromDate = sdfDDMMYYYY.parse(fromDate);
		Date dtTillDate = sdfDDMMYYYY.parse(tillDate);
		// String dateYearDiff =
		// ApplicationDateUtils.calculateDateDifference(dtFromDate, dtTillDate);
		String dateYearDiff = UmtApplicationDateUtils.calculateDateDifferenceModified(dtFromDate, dtTillDate);
	...
	public static String getFinYearAsYYYYtoYYFromMonthYear(int intMonth, int intYear) {
		...
		try {
			if (!String.valueOf(intMonth).equals("") && intMonth != 0 && !String.valueOf(intYear).equals("")
					&& intYear != 0) {
	...
	public static String getFinYearAsYYYYtoYYAsPerCurrDate() {
		...
		try {
			strEndYrInCurrFinYr = strCurrFinYear.substring(0, 2)
					.concat(strCurrFinYear.substring(strCurrFinYear.indexOf("-") + 1));
