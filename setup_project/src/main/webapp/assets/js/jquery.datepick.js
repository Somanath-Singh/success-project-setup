...
monthsOffset: function(date) { // Always start on the quarter
  ...
  alert('Selected ' + dates);
		...
		formatDate: function(format, date, settings) {
			$('.datepick-popup.dp_clickFocus_int').on('click', function(){
       				$('.datepicker_con.datepicker_clickFocus input').focus();
    		});
...
$.datepick.newDate(2014, 12, 25) */
		...
		_attachments: function(elem, inst) {
			...
			$('.datepicker_clickFocus .datepick-trigger').on('click', function(){
        		$(".datepick-popup").addClass('dp_clickFocus_int');
    		});

    		$('.datepicker_clickFocus .is-datepick').on('click', function(){
        		$(".datepick-popup").addClass('dp_clickFocus_int');
    		});
		},
