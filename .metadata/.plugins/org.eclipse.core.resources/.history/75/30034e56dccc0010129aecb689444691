...
<%-- 	<div class="row mt-3">
		<div class="col-lg-12 col-md-12 mb-2">
			<div class="card">
				<div class="card-body d-flex justify-content-between align-items-center pt-1 pb-1">
					<c:set var="startYear" value="${fn:substringBefore(currFinYear, '-')}" />
					<c:set var="endYear" value="${fn:substringAfter(currFinYear, '-')}" />
					<c:set var="shortEndYear" value="${fn:substring(endYear, 2, fn:length(endYear))}" />
					<c:set var="shortYearRange" value="${startYear}-${shortEndYear}" />

          			<h5 class="mb-0">Financial Year: <span id="finSpanId">${shortYearRange}</span></h5>
					  <div class="row align-items-center">
						<div class="col-md-7">
							<label for="financialYear" class="mb-0 form-label">Select Financial Year</label>
						</div>
						<div class="col-md-5">
							<select id="finYearId" class="form-control form-select" onchange="fetchDashboard()">
								<c:forEach items="${finYearList}" var="fin">
									<option value="${fin.finYear}" ${fin.finYear eq finYear ? 'selected' : ''}>${fin.finYear}</option>
								</c:forEach>
							</select>
						</div>
					</div>
					<div class="row align-items-center">
						...
						<div class="col-auto">
							<select id="moduleCode" class="form-control form-select" onchange="fetchDashboard()">
...
<%-- 		<%@ include file="/WEB-INF/views/executive-dashboard/ipms-dashboard.jsp"%> --%>
<%-- 		<%@ include file="/WEB-INF/views/executive-dashboard/hrms-dashboard.jsp"%> --%>
<%-- 		<%@ include file="/WEB-INF/views/executive-dashboard/grievance-dashboard.jsp"%> --%>
<%-- 		<%@ include file="/WEB-INF/views/executive-dashboard/hoarding-dashboard.jsp"%> --%>
<%-- 		<%@ include file="/WEB-INF/views/executive-dashboard/ecashbook-dashboard.jsp"%> --%>
<%-- 		<%@ include file="/WEB-INF/views/executive-dashboard/fams-dashboard.jsp"%> --%>
<%-- 		<%@ include file="/WEB-INF/views/executive-dashboard/rent-collection-dashboard.jsp"%> --%>
	</div>
</div>
