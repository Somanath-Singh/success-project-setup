...
public class CaptchaController implements MessageSourceAware{
	...
	public String switchRole(RedirectAttributes model, @RequestParam Long roleId, @RequestParam(required = false) Long entityId,  @RequestParam(required = false) String appCode, HttpServletRequest request)
	{
		String returnPath =   "redirect:" + POST_LOGIN_URL;
		try
		{
	...
	private void loadMenus(HttpServletRequest request, LoggedInUser liu, String appCode)
	{
		...
		if (appCode != null)
		{
			m_menus = m_menus.stream().filter(e -> e.getAppCode() == null || e.getAppCode().equals(appCode)).collect(Collectors.toList());
		}
	...
	public String changePassword(RedirectAttributes attr, String txtPass, String txtRePass, HttpServletRequest request, HttpServletResponse response) {
		try{
			User user = SecurityHelper.getCurrentUser();
			...
			if(txtRePass != null && !txtRePass.trim().isEmpty() && txtPass != null && !txtPass.trim().isEmpty()) {
				...
				} else throw new Exception("Password does not meet the policy");
			}else throw new Exception("Password not changed");
		}catch(Exception e){
