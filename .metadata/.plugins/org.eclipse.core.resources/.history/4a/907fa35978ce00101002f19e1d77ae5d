...
public class FileExtensionValidator implements ConstraintValidator<FileExtension, MultipartFile> {
    ...
    public boolean isValid(MultipartFile file, ConstraintValidatorContext context) {
        ...
        if (file.getSize() > maxSize) {
            return addConstraintViolation(context, String.format("The file size exceeds the maximum allowed limit of %.2f MB.", maxSizeMB));
        }
        ...
        if (fileExtension.isEmpty() || !allowedExtensions.contains(fileExtension.toLowerCase())) {
            return addConstraintViolation(context, customMessage.isEmpty() ?
                    String.format("The file extension '%s' is not allowed. Allowed extensions are: %s.", fileExtension, String.join(", ", allowedExtensions)) : customMessage);
        }
