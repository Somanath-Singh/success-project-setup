...
public class MenuServiceImpl implements MenuService, MessageSourceAware {
        ...
        try {
            ...
            for (Menu menu : menus) {
                if (menu.getIsActive()) {
                    ...
                    if (role != null) {
                        Optional<Menu> allocated = lstAllocatedMenus.stream().filter(m -> Objects.equals(m.getMenuId(), menu.getMenuId()) && m.getIsActive()).findFirst();
                    ...
                    children = children.stream().sorted(Comparator.comparing(Menu::getDisplayOrder)).collect(Collectors.toList());
    ...
    public List<Privilege> getPriviledgesForRoleAndMenu(Long roleId, Long menuId) {
        ...
        try {
            String query = "select p.*  " + "from t_umt_role_menu_privilege_map rpm " + "join t_umt_role_menu_map rmm " + "on rmm.role_menu_map_id = rpm.role_menu_id " + "join t_umt_mst_privilege p " + "on p.mst_privilege_id = rpm.privilege_id " + "where rmm.role_id = :roleId " + "and rmm.menu_id = :menuId ";
        ...
        try {
            ...
            if (init) {
                ...
                for (RequestMappingInfo rmi : endPoints) {
                    ...
                    Optional<Menu> dbEntry = lstMenus.stream().filter(m -> m.getMenuURL().equals(urlPattern)).findFirst();
    ...
    private List<Menu> getMenusAssingedToRole(Long roleId) {
        ...
        String querySQL = "select m.* " + "from t_umt_menu m " + "join t_umt_role_menu_map rmm  " + "	on rmm.menu_id = m.menu_id " + "	and rmm.role_id = :roleId  " + "	and rmm.is_active = true  " + " where m.is_active = true ";
    ...
    public JSONArray getMobileMenuAssignmentsForRole(Role role) {
        ...
        menus = menus.stream().filter(m ->
                m.getParent() == null).collect(Collectors.toList());
        if (!user.getPrimaryRole().getRoleCode().equals("ROLE_SYSTEM")) {
            menus = menus.stream().filter(m -> !(m.getMenuURL() != null && m.getMenuURL().startsWith("/system"))).collect(Collectors.toList());
        }
        menus = menus.stream().sorted((a, b) -> a.getDisplayOrder().compareTo(b.getDisplayOrder())).collect(Collectors.toList());
        ...
        try {
            ...
            for (Long menuId : menuIds) {
                ...
                if (!rmMap.isPresent()) {
                    ...
                    rmm.setIsActiveForMobile(true);
                    roleMenuMapRepository.save(rmm);
                    logger.debug("Mobile Menu Activated" + menuId + "was " + !rmm.getIsActive() + " from role : " + roleCode);
