...
public class EntityLevelServiceImpl implements EntityLevelService{
    ...
    public Map<String, Object> createAndUpdateUser(String roleCode, String classTypeKey) {
        ...
        Optional<RoleRightLevelMaster> levelMasterByLevelCode = roleService.getRoleRightLevelMasterByLevelKeyClass(classTypeKey);
        if (levelMasterByLevelCode.isPresent()) {
            ...
            if (!roleAndLevel.isPresent()) {
                ServiceOutcome<RoleLevelMap> roleLevelMapServiceOutcome = roleService.addNupdateRoleLevelMap(new Long[]{-1L}, new Long[]{roleRightLevelId}, new Boolean[]{true}, roleId);
    ...
    public Map<String, Object> createUserAsPerNewEntityLevel(String roleCode, String userType, String email,
        ...
        ServiceOutcome<User> muUser = userService.addUser(email, firstName, lastName, null,
                phoneNo, email, roleIds, primaryRoleIds, userType,
                userType, entityLevel, objectId, new Long[]{0L}, code);
    ...
    // recursive call to get all parent object ids and level from parent object id making two list of ids and list of levels
    @Override
    public void getParentObjectIdsAndLevel(Long objectId, String objectLevel, List<CurrentUserVo.AllEntityIdsAndLevel> allEntityIdsAndLevels) {
        try {
            hierarchyEntityMapRepository.findByOrgHirAndEntityIdAndEntityType(objectId, objectLevel).ifPresent(hierarchyEntityMap -> {
                if (hierarchyEntityMap.getParentObjectId() != null && hierarchyEntityMap.getParentObjectType() != null) {
                    ...
                    allEntityIdsAndLevel.setEntityIdAndLevel(hierarchyEntityMap.getParentObjectId() + "##" + hierarchyEntityMap.getParentObjectType());
                    // if all parent object ids and level and IdAndLevel are already present, then return
                    if (allEntityIdsAndLevels.stream().anyMatch(entityIdsAndLevel -> entityIdsAndLevel.getEntityIdAndLevel().equals(hierarchyEntityMap.getParentObjectId() + "##" + hierarchyEntityMap.getParentObjectType()))) {
                        return;
                    }
                    allEntityIdsAndLevels.add(allEntityIdsAndLevel);
                    getParentObjectIdsAndLevel(hierarchyEntityMap.getParentObjectId(), hierarchyEntityMap.getParentObjectType(), allEntityIdsAndLevels);
                }
            });
        } catch (Exception e) {
