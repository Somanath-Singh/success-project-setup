...
<body>
  ...
  <header class="">
    <h2 style="margin:0;">${appLogName} Log Viewer (ENV: ${activeProfile})</h2>
    <div style="text-align: center; padding: 10px;">
      ...
      <button class="dangerBtn" onclick="hardClearLog()">Hard Clear Log (${fetchLogFileSize})</button>
      ...
      <!-- Head clear catlina -->
      <button class="dangerBtn" onclick="hardClearCatLog()">Hard Clear Catlina Log (${fetchCatLogFileSize})</button>
  ...
  <script>
    ...
    async function hardClearLog(){
      let txt = "Are you sure you want to hard clear Application log file ?";
    ...
    async function hardClearCatLog(){
      let txt = "Are you sure you want to hard clear catlina log ?";
      if(confirm()){
        // Show spinner
        showSpinner();
        fetch('${contextPath}/log/hard-clear-cat-logs')
          .then(response => response.json())
          .then(data => {
            document.getElementById('logContent').value = data.join('\n');
            document.getElementById('logContent').scrollTop = document.getElementById('logContent').scrollHeight;
