...
function fileCheckDocument(that) {
    // Allowed file extensions
  	var ValidFileExtension = ['png','jpg','pdf','jpeg'];

	if($(that).val().split('.').length == 2 ) {
		if ($.inArray($(that).val().split('.').pop().toLowerCase(), ValidFileExtension) == -1) {
			bootbox.alert("Sorry! allowed format is png, Jpg ,jpeg or pdf only.");
			$(that).val("");
			return false;
		}
		...
		$.ajax({	
			...
			success : function(ajaxResp) 
			{
				...
				if(data != null && data != "")
				{
					...
					$("#aadharNo").text(data.aadharNo ? maskString(data.aadharNo,2,10) : "N/A");
					$("#panNo").text(data.panNo ? maskString(data.panNo,2,8) : "N/A");
					...
					$("#college").text(data.collegeName ? data.collegeName : "N/A");
					$("#department").text(data.departmentName ? data.departmentName : "N/A");
					...
					$("#hdnPayCommsnId").val(data.payCommsnId);
					...
					{
						$("#profileImage").attr("src", "data:image/jpg;base64, "+data.profilePicBase64);
						$("#profileImageDiv1").hide();
						$("#profileImageDiv2").hide();
						$("#profileImageDiv3").show();
					}
					else
					{
						$("#profileImageDiv1").hide();
						$("#profileImageDiv2").show();
						$("#profileImageDiv3").hide();
					}
...
function AJAX_checkIfAttendanceProcessedForCollegeMonthYearEmplmntType(objTypeId, monthNum, yearNum, emplmntTypeId,objType)
{
	var ajaxResponse = "";
	
	if(objTypeId != "" && monthNum != "0" && monthNum != "" && yearNum != "0" && yearNum != "" && emplmntTypeId != "" && emplmntTypeId != "0" && objType != "")
	{
		showLoader();
		$.ajax({	
...
function AJAX_generatePaybillAndGetOtherDetailsBySalTypeColgIdMonthYear(salaryType, objTypeId, monthNum, yearNum, emplmntTypeId,objType)
{
	var ajaxResponse = "";
	
	if(salaryType != "" && objTypeId != "" && monthNum != "0" && monthNum != "" && yearNum != "0" && yearNum != "" && emplmntTypeId != "" && emplmntTypeId != "0" && objType != "")
	{
		showLoader();
		$.ajax({	
...
//create a class to validate the date means it should be 18 yr gap from current date atleast
function validateDateOfBirth(that){
	var dob = $(that).val();
	var age = calculateAge(dob);
	if(age < 18){
		$(that).val('');
		bootbox.alert("Age should be atleast 18 years");
	}
}
...
// Apply the numbers-only restriction to all input fields with the 'numberOnly' class
...
//creatae a class on implememtation will disable today on wards date in input type date 
$(".disableFutureDate").on("change", function() {
	var today = new Date().toISOString().split('T')[0];
	var value = $(this).val();
	if(value > today){
		$(this).val('');
		bootbox.alert("Date should not be greater than today");
	}
	});

//Restrict copy paste
$(".restrict-copy-paste").on("paste copy cut", function(event) {
	event.preventDefault(); // Prevent copy, paste, and cut
});

//Restrict keyboard input
$(".restrict-input").on("keypress", function(event) {
	event.preventDefault(); // Prevent key press
});


//common date validation
function commonDateValidation(dob_session , entryDate ){
	var dob_session1 = "${sessionScope.dob_session}";
	var entryDate = entryDate;
	//difference should be 18 years minimum
	var dob = new Date(dob_session);
	var entry = new Date(entryDate);
	var difference = entry.getFullYear() - dob.getFullYear();
	if(difference < 18){
		bootbox.alert("Entry Date should be 18 years from Date of Birth");
		return false;
	}
