...
<sec:authentication var="principal" property="principal" />
		...
		<nav class="navbar navbar-expand px-2">
                ...
                <button class="btn sidebarBtn" type="button" data-bs-theme="dark" style="margin-right: 10px;">
                    <span class="fa-solid fa-bars"></span>
                </button>
                <div class="logo-section">
                	<img src="${contextPath}/assets/images/favicon.png" style="width: 100%;">
                </div>
                <div class="d-flex flex-column home-content">
			        <span class="text"> Building &amp; Asset Management System</span>
			        <p class="my-0" style=" border-top: #ffffff 2px solid;"><small>ST &amp; SC Development, Minorities &amp; Backward Classes Welfare Dept. Govt. of Odisha</small>
			        </p>
			      </div>
                <a href="javascript:void()" class="expandBtn ms-3 mt-1" onclick="toggleFullscreen()" title="Toggle Fullscreen" style="">
                    <i class="fa-solid fa-expand"></i>
                </a>

                <a href="javascript:void()" class="expandBtn ms-3 mt-1" id="restoreMessage" title="Restore Message" style="display: none;">
                  <i class="fa-solid fa-expand"></i>
              </a>
                ...
                <ul class="riteside">
					<li class="select-role">
					 <label for="roleSwitcher" class="form-label" style="color: #ffffff;font-size: 14px;line-height: 10px;">Role:</label>
                        <select class="form-control" name="role" id="roleSwitcher"> 
							<c:forEach items="${principal.dbUser.roles}" var="role">
								<option value="${role.roleId}" ${role.roleId eq principal.dbUser.primaryRole.roleId ? 'selected': '' }>${role.displayName}</option>
							</c:forEach>
						</select>
                    </li>
					<!-- If principal.currentUserVo.dynamicEntities is size one then make readonly  -->
<!--                     <li class="select-role "> -->
					<li class="select-role" style="display: none;">
                      <label for="entitySwitcher" class="form-label" style="color: #ffffff;font-size: 14px;line-height: 10px;">Entity:</label>
                        <select class="form-control" name="role" id="entitySwitcher">
							<option value="">Select Entity</option>
							<c:forEach items="${principal.currentUserVo.dynamicEntities}" var="entite">
								<option value="${entite.entityIds}" ${entite.entityIds eq principal.currentUserVo.entityIdStr ? 'selected': '' }>${entite.entityName}</option>
							</c:forEach>
						</select>
                    </li>
                    <li class="dot-icon">
                          ...
                          <div id="icon-menu" class="menu hidden">
                            <c:forEach var="module" items="${USER_MODULES}">
                            			<div class="col-md-4 module-data">
                            				<a href="#" class="single-small-card" onclick="navigateToModule('${module.moduleCode}' , ${module.isUnderDevelopment})">
                    ...
                    <%-- <li class="dropdown activeUser">
                        <img src="${contextPath}/assets/images/profile.png" alt="Profile">
                        <div class="dropdown-toggle" data-bs-toggle="dropdown">
                            <p>${principal.dbUser.firstName} ${principal.dbUser.lastName}</p>
                            <span>${principal.primaryRole.displayName}</span>
                        </div>
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuLink" style="box-shadow: 0px 0px 10px 0px #003a7a !important;">
						    <!-- <li> <a class="dropdown-item" href="#"><i class="fa-regular fa-user"></i>My Profile</a></li>  -->
						     <!-- <li><a class="dropdown-item" href="#"><i class="fa-solid fa-gear"></i>Setting</a></li>  -->
						         <li><a class="dropdown-item" href="#"  onclick="changePasswordModalOpen()" ><i class="fa-solid fa-gear"></i>Change Password</a></li> 
								 <!-- Back to Home -->
							<!-- <li><a class="dropdown-item" href="${contextPath}/home"><i class="fa-solid fa-gear"></i>Back To Home</a></li>  -->
    						<li class="d-flex justify-content-center"></li>
						  </ul>
                    </li> --%>
                    <li> <a href="#" id="switchlogout" type="button" class=""><img src="${contextPath}/assets/images/loginPage/signout.png" alt="signout" class="signout" title="Sign Out"></a></li>
                </ul>
            </nav>
    ...
    <div class="modal-content">
	  ...
	  <div class="col-md-12" style="background-color: burlywood;">
		...
		<ul class="">
			...
			<li>Contains at least one special character from the list:
				<br>!#$%&'()*+,-./:;<=>?@[]^_`{|}
			</li>
			<li>Length must be between 8 and 15 characters</li>
		</ul>
		<p style="color: green; font-weight: bold;">Status: Valid/Invalid based on your input</p>
	</div>
      </div>
...
<form method="post" action="${contextPath}/overwrite/umt/switchRole" id="frmSwitchRole">
	<input type="hidden" name="${_csrf.parameterName}" 	value="${_csrf.token}" />
	<input type="hidden" name="roleId" id="hdnRoleId" />
	<input type="hidden" name="entityId" id="hdnEntityId" />
</form>

<form method="post" action="${contextPath}/umt/user/captcha/change/password" id="frmChangePassword">
	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
	<input type="hidden" name="txtPass" id="newPasswordId" />
	<input type="hidden" name="txtRePass" id="confirmPasswordId" />
</form>
...
function toggleFullscreen() {
    if (!document.fullscreenElement) {
        ...
        document.getElementById('restoreMessage').style.display = 'block';
    } else {
        document.exitFullscreen();
        sessionStorage.removeItem('isFullscreen');
        document.querySelector('.expandBtn').style.display = 'block';
